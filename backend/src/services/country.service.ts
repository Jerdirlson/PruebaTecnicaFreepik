...
/**
 * Metodo para obtener todas las countrys.
 */

export async function getCountryService(res : Response) {
    try {
        const db: DataBase = await initDatabase(res)
        const query = 'SELECT country_id , name FROM Country';
        const result : Country[] = await db.readQuery(query, [])
        console.log(result);
        return { success: true, message: 'Countrys found.', data: result };
    } catch (error) {
        console.error('Error getting countrys:', error);
        return { success: false, message: 'Error getting countrys.' };
    }
}
